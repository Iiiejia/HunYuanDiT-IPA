...
IMAGE_PROMPT_PATH = "/models/dataset/porcelain/images/0.png"
IPA_SCALE = 1.0
SEED = 0
PROMPT = """
a pictrue of porcelain. 一张瓷器的图片。
"""
IPA_MODEL_PATH = "/models/ipa/last"
...
if __name__ == "__main__":
    ...
    with torch.inference_mode(True), torch.no_grad():
        ...
        (
            ...
            mt5_embedder,
            vae,
        ) = load_model("/models/HunyuanDiT-v1.2/t2i", dtype=DTYPE, device=DEVICE,
                       dit_path="/models/save/porcelain.ckpt",
                       )
        ...
        ipa_model = IPAModel(denoiser.blocks, DEVICE, DTYPE)
        ipa_model.negative = True
        ipa_model.load_state_dict_from_folder(IPA_MODEL_PATH)
        ipa_model.set_ip_emb_with_path_list([IMAGE_PROMPT_PATH])
        ipa_model.set_ip_scale(1.0, IPA_SCALE)
